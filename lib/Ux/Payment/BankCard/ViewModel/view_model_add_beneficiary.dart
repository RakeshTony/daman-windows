...
class ViewModelAddBeneficiary extends BaseViewModel {
  ...
  void requestAddBeneficiary(
    ...
    if (_error.isEmpty) {
      HashMap<String, dynamic> requestMap = HashMap();
      requestMap["id"] = Encoder.encode(_id);
      requestMap["bank_id"] = Encoder.encode(_bankId);
      requestMap["acc_type"] = Encoder.encode(_accType);
      requestMap["acc_number"] = Encoder.encode(_accountNumber);
      requestMap["f_name"] = Encoder.encode(_firstName);
      requestMap["l_name"] = Encoder.encode(_lastName);
      requestMap["payment_method"] = Encoder.encode(_paymentMethod);
      requestMap["phone"] = Encoder.encode(_mobile);
      requestMap["email"] = Encoder.encode(_email);
      requestMap["is_beneficiary"] = Encoder.encode(beneficiaryType.toString());
      request(
        networkRequest.addBeneficiary(requestMap),
        (map) {
